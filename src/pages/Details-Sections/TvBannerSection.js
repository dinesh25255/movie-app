...
const TvBannerSection = ({ data, setPopup, showTrailer }) => {
    ...
    <div className='w-[100%] flex px-10 img-wrapper  bg-blend-darken bg-no-repeat bg-cover  h-[100%] py-10 bg-center rj-sm:block rj-sm:px-5 rj-sm:py-2 rj-md:pl-2' style={{ backgroundImage: data?.backdrop_path && data?.backdrop_path !== null && data?.backdrop_path !== undefined && `url('https://image.tmdb.org/t/p/w600_and_h900_bestv2${data?.backdrop_path}')` }}>
      ...
      <div className="hidden rj-sm:block w-[100%]">
        <h2 className='text-lg text-center font-semibold text-teal-400 rj-sm:pb-3'>{data?.name ?? data?.orginal_name}
          {data?.first_air_date != null ? <span> ( {data?.first_air_date.split('-')[0]} )  </span> : ""}
        </h2>
      </div>
      ...
      <div className="right w-[50%] rj-sm:w-[100%] rj-2xl:w-[70%] rj-3xl:w-[60%] ">
        ...
        <h2 className='text-2xl font-semibold text-teal-400 rj-sm:hidden'>{data?.name ?? data?.orginal_name}
          {data?.first_air_date != null ? <span> ( {data?.first_air_date.split('-')[0]} )  </span> : ""}
        </h2>
        <div className="py-2">
          ...
          <div className=" text-base font-medium text-teal-50 rj-sm:text-sm ">
            ...
            <h3 className='mt-1 rj-sm:text-center rj-sm:text-sm'>Released Date : {data?.first_air_date}</h3>
        ...
        <div className="flex items-center space-x-6 py-3 rj-sm:space-x-4 rj-sm:text-sm  rj-sm:justify-center  ">
          ...
          {showTrailer && < div className="play-wrapper flex items-center space-x-2 hover:text-teal-400 cursor-pointer " onClick={() => {
            setPopup(true);
        ...
        <div className="flex justify-between w-[40%] pt-3 rj-sm:w-[80%] rj-sm:text-sm rj-sm:pb-3 rj-md:w-[60%] rj-lg:w-[50%]">
          <div className="">
          ...
          <div className="">
            <h2>Season : {data?.number_of_seasons}</h2>
            <h2>Episode : {data?.number_of_episodes}</h2>
          </div>
...
export default TvBannerSection